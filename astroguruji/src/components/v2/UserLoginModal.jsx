  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/user_api/user_login_new`, {
        ...
        body: JSON.stringify({ number: mobile, country_code: dialCode, country: "INR", otp: "", type: "" }),
      });
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/user_api/user_login_new`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token ?? ""}` },
        body: JSON.stringify({ number: mobile, country_code: dialCode, country: "INR", otp: finalOtp, type: "" }),
      });
      const data = await res.json();
      if (data.status === true) {
        ...
        onClose();
        // ✅ Refresh so all components (Navbar, wallet, user state) reflect logged-in state
  ...
  return (
    ...
    >
      <div className="w-full md:max-w-md bg-white rounded-t-2xl md:rounded-2xl p-6 animate-slideUp">
        ...
        {step === "mobile" && (
          <>
            <p className="text-gray-500 text-sm mb-4">Enter your mobile number to continue</p>
            <div className="flex items-center border rounded-lg px-3 py-2 mb-4 focus-within:ring-2 focus-within:ring-orange-400">
              ...
              <input
                ...
                className="w-full outline-none text-sm" placeholder="Enter mobile number" autoFocus
              />
            </div>
            <button onClick={sendOtp} disabled={isLoading}
              ...
              {isLoading
                ? <span className="flex items-center justify-center gap-2"><span className="h-4 w-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Sending OTP…</span>
                : "Send OTP"}
            </button>
        ...
        {step === "otp" && (
          <>
            ...
            <button onClick={verifyOtp} disabled={isLoading}
              className="w-full bg-orange-500 hover:bg-orange-600 disabled:opacity-60 text-white py-3 rounded-lg font-medium transition-colors">
              {isLoading
                ? <span className="flex items-center justify-center gap-2"><span className="h-4 w-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Verifying…</span>
                : "Verify OTP"}
            </button>
            <div className="text-center mt-4 text-sm text-gray-500">
              {timer > 0
                ? <>Resend OTP available in <span className="font-medium">{timer}s</span></>
                : <button onClick={resendOtp} className="text-orange-500 font-medium hover:underline">Resend OTP</button>}
            </div>
            <button onClick={() => { setStep("mobile"); setError(""); setOtp(["", "", "", ""]); }}
