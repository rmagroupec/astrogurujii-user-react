...
type ApiAstrologer = {
  ...
  online?: boolean; // optional — API may not return this
};
...
export default function ConsultantCardNew({
  ...
  const offerPrice = consultant.per_min_chat_offer
    ? parseFloat(consultant.per_min_chat_offer)
    : null;

  // Show original (crossed) price: if offer exists use price as original, else show a fake ~20% higher
  ...
  const specialty =
    consultant.language?.map((l) => l.name).join(", ") || "Astrology";
    ...
    <div onClick={() => navigate(`/consultants/${consultant.id}`)} className="group relative w-full rounded-[10px] border border-[#DADADA] bg-white transition-all hover:border-brand-amber">
      ...
      <div className="px-[10px] sm:px-[14px] pb-2 pt-[55px] sm:pt-[80px] text-center">
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          {hasDiscount && (
            <div className="absolute right-[9px] top-[9px] z-10 rounded-[5px] bg-[#FF6F00] px-2 py-1 font-poppins text-[9px] font-bold text-white">
              {discountPct}% OFF
            </div>
          )}
        </div>
      ...
      {/* Experience row — replaces rating/reviews since API has no rating */}
          ...
          <div className="flex items-center gap-1">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M7.78381 1.3297L8.15141 2.28502C8.16245 2.31375 8.18819 2.33218 8.21927 2.33367L9.25363 2.38343C9.32397 2.38681 9.35278 2.47427 9.29787 2.5178L8.49069 3.15798C8.46643 3.17722 8.4566 3.20708 8.46476 3.2367L8.73645 4.22275C8.75492 4.2898 8.67956 4.34387 8.62062 4.30578L7.75416 3.74612C7.72811 3.72931 7.69633 3.72931 7.67028 3.74612L6.80382 4.30581C6.74491 4.34387 6.66953 4.28983 6.68799 4.22278L6.95968 3.2367C6.96784 3.20705 6.95804 3.17722 6.93375 3.15798L6.12657 2.5178C6.07166 2.47427 6.10047 2.38681 6.17081 2.38343L7.20517 2.33367C7.23628 2.33218 7.26199 2.31375 7.27303 2.28502L7.6406 1.3297C7.66549 1.2651 7.75898 1.2651 7.78381 1.3297Z"
                fill="#FF6F00"
              />
            </svg>
            <span className="font-euclid text-[10px] text-text-subtle whitespace-nowrap">
